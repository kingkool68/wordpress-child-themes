...
function daddio_infinite_scroll_rewrite_rules() {
	add_rewrite_endpoint( 'pages', EP_ALL );
}
add_action('init', 'daddio_infinite_scroll_rewrite_rules');

//
function daddio_infinite_scroll_pre_get_posts( $query ) {
    if ( is_admin() || !$query->is_main_query() ) {
        return;
	}

	if ( $query->is_archive() && ( $pages = daddio_infinite_scroll_get_pages() ) ) {
		$start = $pages->start;
...
add_action( 'pre_get_posts', 'daddio_infinite_scroll_pre_get_posts' );

// Adjust the limit clause of the main SQL query
function daddio_infinite_scroll_post_limits( $limit ) {
	global $wp_query;
	if ( is_admin() || !$wp_query->is_main_query() ) {
		return $limit;
	}

	if( $pages = daddio_infinite_scroll_get_pages() ) {
		if( $pages->start == -1 ) {
			return $limit;
		}
...
add_filter( 'post_limits', 'daddio_infinite_scroll_post_limits' );

function daddio_infinite_scroll_wp() {
	global $wp_query, $paged;
	$pages = daddio_infinite_scroll_get_pages();
	if( !$pages || !$pages->end ) {
		return;
	}
...
add_action( 'wp', 'daddio_infinite_scroll_wp' );

function daddio_infinite_scroll_redirect_canonical( $redirect_url, $requested_url ) {
...
add_filter( 'redirect_canonical', 'daddio_infinite_scroll_redirect_canonical', 10, 2 );

function daddio_infinite_scroll_get_pagenum_link( $result ) {
	if( $pages = daddio_infinite_scroll_get_pages() ) {
...
add_filter( 'get_pagenum_link', 'daddio_infinite_scroll_get_pagenum_link' );

// Determine if the request should load the infite scroll script or not
function daddio_infinite_scroll_enqueue_script() {
...
add_action( 'wp_enqueue_scripts', 'daddio_infinite_scroll_enqueue_script' );
...
function daddio_infinite_scroll_get_pages() {
	global $wp;
	...
	if( property_exists( $wp, 'daddio_infinite_scroll_page_details' ) && !empty( $wp->daddio_infinite_scroll_page_details ) ) {

		return $wp->daddio_infinite_scroll_page_details;
	}
	...
	$wp->daddio_infinite_scroll_page_details = $output;
