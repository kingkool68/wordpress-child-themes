 ...
 * @return [boolean]
 */
function daddio_use_global_script_file() {
	if ( is_admin() ) {
		return false;
	}
 ...
 */
function daddio_wp_enqueue_scripts() {
	// CSS
	$css_suffix = '.min.css';
	...
	if( daddio_use_global_script_file() ) {
		add_filter( 'script_loader_tag', 'daddio_dont_load_bundled_scripts', 10, 3 );
		wp_enqueue_script( 'zah-global-scripts' );
	}
}
add_action( 'wp_enqueue_scripts', 'daddio_wp_enqueue_scripts' );
 ...
 */
function daddio_style_loader_tag( $html, $handle, $href, $media ) {
...
add_filter( 'style_loader_tag', 'daddio_style_loader_tag', 10, 4 );
 ...
 */
function daddio_dont_load_bundled_scripts( $script_element, $handle, $script_src ) {
	if( !daddio_use_global_script_file() ) {
 ...
 * @return [string]						New <script> element
 */
function daddio_rejigger_jquery( $script_element, $handle, $script_src ) {
	if( is_admin() ) {
		return $script_element;
	}
...
add_filter( 'script_loader_tag', 'daddio_rejigger_jquery', 10, 3 );
 ...
 */
function daddio_move_head_scripts_to_footer() {
...
add_action( 'after_setup_theme', 'daddio_move_head_scripts_to_footer' );
